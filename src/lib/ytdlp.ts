...
const TOUKIR_DIR = path.join(os.homedir(), '.toukir', 'bin')
...
export async function ensureYtDlp(onStatus: (message: string) => void, signal?: AbortSignal): Promise<string> {
  ...
  const local = path.join(TOUKIR_DIR, process.platform === 'win32' ? 'yt-dlp.exe' : 'yt-dlp')
  if (await commandWorks(local, ['--version'])) return local

  onStatus('setting up — fetching yt-dlp…')
  await fs.mkdir(TOUKIR_DIR, {recursive: true})

  const url = `${RELEASE_BASE}/${ytDlpAssetName()}`
  const response = await fetch(url, {signal})
  if (!response.ok || !response.body) {
    throw new Error(`Could not download yt-dlp (HTTP ${response.status}).`)
  }
...
export async function probe(ytdlp: string, url: string, signal?: AbortSignal): Promise<ProbeResult> {
  ...
  try {
    ...
    throw new Error('yt-dlp returned video info that could not be read.')
  }

  const infoJsonPath = path.join(os.tmpdir(), `toukir-info-${process.pid}-${Date.now()}.json`)
...
export function buildChoices(info: VideoInfo): DownloadChoice[] {
  ...
  const audioSize = bestAudio ? estimateFormatSize(bestAudio, info.duration) : 0
  ...
  for (const height of heights.slice(0, MAX_VIDEO_CHOICES)) {
    ...
    const best = [...candidates].sort((a, b) => scoreVideo(b, info.duration) - scoreVideo(a, info.duration))[0]
    const muxed = best.acodec && best.acodec !== 'none'
    const videoSize = estimateFormatSize(best, info.duration)
    const size = videoSize + (muxed ? 0 : audioSize)
    // a total built only from the audio track would read as the whole file's
    // size — show no size at all instead when the video part is unknown
    const sizeLabel = videoSize > 0 ? ` · ~${formatBytes(size)}` : ''
    choices.push({
      kind: 'video',
  ...
  const audioSizeLabel = audioSize > 0 ? ` · ~${formatBytes(audioSize)}` : ''
  choices.push({
    kind: 'audio',
...
/**
 * Bytes for one format: exact size, then yt-dlp's rough estimate, then
 * bitrate × duration — the same rule yt-dlp uses to derive filesize_approx.
 * Several extractors (notably current YouTube itags) ship tbr but no size
 * fields; without the bitrate fallback every resolution showed the same
 * audio-only size.
 */
function estimateFormatSize(f: RawFormat, duration?: number): number {
  if (f.filesize && f.filesize > 0) return f.filesize
  if (f.filesize_approx && f.filesize_approx > 0) return f.filesize_approx
  if (f.tbr && f.tbr > 0 && duration && duration > 0) return Math.round((f.tbr * duration * 1024) / 8)
  return 0
}

function scoreVideo(f: RawFormat, duration?: number): number {
  let score = f.tbr ?? 0
  if (f.ext === 'mp4') score += 10_000
  if (f.vcodec?.startsWith('avc')) score += 5_000
  // the label's source format only affects the picker display — the -f
  // selector still decides what actually downloads. Prefer a candidate whose
  // size we can show over a better-bitrate one we can't size at all.
  if (estimateFormatSize(f, duration) > 0) score += 100_000
  return score
}
